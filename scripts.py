...
# VOLUME_CHOICES = ['100ML', '250ML', '500ML', '1L', '2L']
...
def createVendorProducts():
    ...
    if not vendor:
        print("❌ No Shopkeeper found. Cannot create VendorProducts.")
        return

    for product in Products.objects.all():
        price = random.randint(100, 10000)
        if VendorProducts.objects.filter(product=product, shopkeeper=vendor).exists():
            continue
...
# uploadImages("C:\\Users\\MANISH_kR\\Ecommerce\\ecommerce\\images")
